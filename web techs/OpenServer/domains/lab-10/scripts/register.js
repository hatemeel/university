...
const validateForm = ({ showValidation = true } = {}) => {
  let valid = true;
...
const openPreviewModal = () => {
  const { controls } = validateForm({ showValidation: false });
        ...
        `
              ...
              (key === 'photo' && photoPreview
                ? `
									...
									`
                : controls[key] && controls[key].length && controls[key]) ||
              'No data provided'
            }</td>
					</tr>
				`,
